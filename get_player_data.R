...
get_player_data <- function(session, n=250, by_position=FALSE, year){
  ...
  if(!as.character(year) %in% c("2013", "2014", "2015", "2016", "2017")){
    stop("year must be one of 2013, 2014, 2015, 2016, or 2017")
  }
  ...
  format_df_off <- function(df){
    df <- df[, -c(1,3,ncol(df))]
    names(df) <- c("player_info", "owner", "games_played", 
                   "bye", "fan_pts", 
                   "perc_owned", "rank_proj", "rank_act", "pass_comp",
                   "pass_yds", 
                   "pass_td", "pass_int", "pass_sack",
                   "rush_att", "rush_yds", "rush_td", 
                   "rec_tgt", "rec_rec", "rec_yds", "rec_tds", "ret_yds",
                   "ret_td", 
                   "twopts", "fum_lost")
    df <- df[-1, ]
  ...
  format_df_def <- function(df){
    df <- df[, -c(1,3,ncol(df))]
    names(df) <- c("player_info", "owner", "games_played", "bye", 
                   "fan_pts", 
                   "perc_owned", "rank_proj", "rank_act", "pts_vs", 
                   "sack", "safe", "int", "fum_rec", 
                   "td", "blk_kick", "ret_yds", "ret_td")
    df <- df[-1, ]
    df$player_name <- stringr::str_extract(df$player_info,
